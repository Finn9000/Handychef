...
class SubscriptionController extends Controller
{
    ...
    public function show(): View
    {
        $subscriptions = auth()->user()->subscriptions()
            ->with('mealPlan.ghostKitchen', 'subscriptionItems.mealItem', 'pickupSchedules')
            ->where('status', 'active')
            ->latest()
            ->get();
    ...
    /**
     * Subscribe the customer to a meal plan, with their chosen meal items.
     */
    public function store(Request $request, MealPlan $mealPlan): RedirectResponse
    {
        ...
        $validated = $request->validate([
            'meal_item_ids' => ['required', 'array', 'min:1'],
            'meal_item_ids.*' => ['integer', 'exists:meal_items,id'],
        ...
        // Make sure every selected item actually belongs to this plan.
        $validItemIds = $mealPlan->mealItems()->pluck('id');
        $selectedIds = collect($validated['meal_item_ids'])->intersect($validItemIds);

        if ($selectedIds->isEmpty()) {
            return back()->withErrors(['meal_item_ids' => 'Please select at least one valid meal item.']);
        }
        ...
        $subscription->subscriptionItems()->createMany(
            $selectedIds->map(fn ($id) => ['meal_item_id' => $id])->all()
        );
